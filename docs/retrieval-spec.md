...
- `studio/src/app/standards/` — the unified "Documents" library (`StandardsLibrary.tsx`): a multi-select document picker (tick one or more; union of their clauses) plus a block-type filter (paragraph/heading/table/definition/list) with live counts, over the full clause list. Documents carry catalogue metadata + supersession. `/clauses` and `/standards/[id]` redirect here.
